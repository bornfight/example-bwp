...
class ThemeUpdateChecker {
	...
	public $metadata_url = '';        //The URL of the theme's metadata file.
	public $enable_automatic_checking = true; //Enable/disable automatic update checks.
	
	protected $option_name = '';      //Where to store update info.
	 ...
	 */
	public function __construct( $theme, $metadata_url, $enable_automatic_checking = true ) {
		$this->metadata_url = $metadata_url;
		if ( strpos( get_site_url(), 'staging' ) ) {
			$this->metadata_url .= '&staging=1';
		}
		
		$this->enable_automatic_checking = $enable_automatic_checking;
		$this->theme                     = $theme;
	...
	public function install_hooks() {
		...
		if ( $this->enable_automatic_checking ) {
			add_filter( 'pre_set_site_transient_update_themes', [ $this, 'on_transient_update' ] );
		}
		...
		add_action( 'delete_site_transient_update_themes', [ $this, 'delete_stored_data' ] );
	}
	...
	public function request_update( $query_args = [] ) {
		...
		$query_args                      = apply_filters( self::$filter_prefix . 'query_args-' . $this->theme, $query_args );
		
		//Various options for the wp_remote_get() call. Themes can filter these, too.
		$options = [
			'timeout' => 10, //seconds
		];
		$options = apply_filters( self::$filter_prefix . 'options-' . $this->theme, $options );
		...
		if ( ( $code == 200 ) && ! empty( $body ) ) {
			$theme_update = ThemeUpdate::from_json( $body );
			//The update should be newer than the currently installed version.
			if ( ( $theme_update != null ) && version_compare( $theme_update->version, $this->get_installed_version(), '<=' ) ) {
				$theme_update = null;
			}
		}
		
		$theme_update = apply_filters( self::$filter_prefix . 'result-' . $this->theme, $theme_update, $result );
	...
	public function add_query_arg_filter( $callback ) {
		add_filter( self::$filter_prefix . 'query_args-' . $this->theme, $callback );
	}
	...
	public function add_http_request_arg_filter( $callback ) {
		add_filter( self::$filter_prefix . 'options-' . $this->theme, $callback );
	}
	...
	public function add_result_filter( $callback ) {
		add_filter( self::$filter_prefix . 'result-' . $this->theme, $callback, 10, 2 );
	}
}
